...
As depicted in Fig. \ref{fig:road_to_op}, we now detail the different ways ((a), (b) and (c))\footnote{\textcolor{red}{Change alpahnumeric to roman}} in which a malicious device obtains \oportunity{}.
...
\textcolor{red}{We dont use type c vulnerability, which drivers have it?}
...
Namely, the drivers do not fill the \texttt{frags}, which the attacker uses to obtain \means{}. Both drivers, use the \texttt{napi\_gro\_receive} function to pass the \skb{} to the upper layer (this is the standard for most NIC drivers \SV{proove!}). 
