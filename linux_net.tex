...
\textcolor{olive}{In this section we discuss the Linux network stack where \oportunity is seemingly missing, as 
\st{Presumably,}} a correct use of the DMA API should thwart the attack outlined in the previous section (Fig. \ref{fig:sh_info}). Namely, first unmapping the buffer and only then initializing the \shinfo{} should allow the CPU to undo any malicious changes the NIC may have perpetrated. \textcolor{olive}{But, as we demonstrate \oportunity is often easily achievable even after the CPU has made its changes.}

\begin{enumerate}[label=(\roman*),wide, labelwidth=!, labelindent=0pt]

\item \st{Presumably, a correct use of the DMA API should thwart the attack outlined in the previous section} (Fig. \ref{fig:sh_info}).\st{Namely, first unmapping the buffer and only then initializing the \shinfo{} should allow the CPU to undo any malicious changes the NIC may have perpetrated.} 
...
\textcolor{olive}{As we have shown a NIC can gain \oportunity fairly easily (Fig.~\ref{fig:road_to_op}).
Also,} a malicious device can generate a poisoned ROP stack on each RX buffer and gain \motivation{}. At this point, the device still cannot execute a successful code injection attack since it is lacking \means{}. 
