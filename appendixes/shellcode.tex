...
To open the new shell, we used the built-in functionality the Linux kernel provides for spawning user processes with root privileges. This command opens a local shell.
...
page[0xFF8] = KERNEL_BASE + 0x003795; // leave ; ret
        \end{lstlisting}
        \caption{
                The pivoting code uses the \texttt{rdi} register, which points to the \means{} page}
