...
To open the new shell, we used the built-in functionality the Linux kernel that provides for spawning user processes with root privileges. This command opens a local shell.
...
ret
        \end{verbatim}
        \caption{Shellcode for spawning a new local root shell.}
...
page[0xFF8] = KERNEL_BASE + 0x003795; // leave ; ret
        ...
        %\vspace{-5mm}
        \caption{
                ROP attack, based on the same Linux functionality to open a remote shell on the attacker's machine. The pivoting code uses \texttt{rdi} register, which points to the \means{} page.}
