...
In particular, the Linux kernel has \DIFdelbegin \DIFdel{a predetermined }\DIFdelend \DIFaddbegin \DIFadd{predetermined ranges for its }\DIFaddend virtual memory layout~\cite{mem_layout}. This layout defines the location of the kernel text mapping, the direct mapping of physical memory and the virtual memory\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{, as depicted in Tab.~\ref{fig:mem_layot}. }\DIFaddend KASLR randomizes the offset of these segments \DIFaddbegin \DIFadd{in the corresponding range on }\DIFaddend each boot.
