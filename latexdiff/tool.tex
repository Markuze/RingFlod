...
\begin{enumerate}
    ...
    \item Type C: Looks for functions that \DIFdelbegin \DIFdel{are used for }\DIFdelend \DIFaddbegin \DIFadd{offer }\DIFaddend fast allocation by slicing a contiguous memory buffer into segments (e.g., netdev\_alloc\_skb, napi\_alloc\_skb). These may result in multiple \iova mapping the same page. These functions utilize the \texttt{page\_frag} API, which we discuss in greater detail in \DIFdelbegin \DIFdel{Sec. \ref{sec:shinfo_exploit}}\DIFdelend \DIFaddbegin \DIFadd{Section \ref{sec:shinfo_exploit}. 
