...
The output starts from the impact evaluation (e.g., detected exposed callback pointers) and continues with pertinent code lines. %\sout{Starred lines contain script analysis}.
...
In Line 6, \tool, after finding the variable declaration (i.e., line 5) and looking at the mapped pointer (i.e., \texttt{\&op->rsp\_iu} on line 4), concludes that the whole \texttt{struct nvme\_fc\_fcp\_op} is exposed to the device. Lines \DIFdelbegin \DIFdel{3-1 }\DIFdelend \DIFaddbegin \DIFadd{1--3 }\DIFaddend repeat the same analysis process for the \texttt{dma\_map\_single} call that exposes the data structure to the device. 

This example demonstrates the recursive nature of the script.
\tool first identifies the suspect function call, then finds the mapped pointer's declaration, and lastly prints its type. This pattern repeats itself in lines \DIFdelbegin \DIFdel{1-3 and 4-6 }\DIFdelend \DIFaddbegin \DIFadd{1--3 and 4--6 }\DIFaddend until a vulnerability is discovered. The findings are then displayed; line 7 counts the number of directly exposed callback pointers, and line 8 displays the number of callback pointers that may be potentially spoofed.
...
In addition to type (a) and (b) vulnerabilities, \tool has additionally flagged 344 cases where a type (c) vulnerability is present. Our analysis also finds three instances where the stack pointer is mapped, potentially simplifying the execution of a ROP attack.

In total, we find 742 dma-map calls (i.e., 72.8\% of all dma-map calls) with a potential vulnerability, of which 344 also admit a type (c) vulnerability.
