...
This section organizes and categorizes the risks associated with DMA operations, providing the building blocks for reasoning about \DIFdelbegin \DIFdel{code-injection }\DIFdelend \DIFaddbegin \DIFadd{code injection }\DIFaddend attacks.
...
\subsection{\DIFdelbegin \DIFdel{Vulnerabilities }\DIFdelend \DIFaddbegin \DIFadd{Vulnerability }\DIFaddend Attributes for Code Injection}\label{sec:mmo}

We introduce a schema that allows for a systematic analysis of \DIFdelbegin \DIFdel{code-injection }\DIFdelend \DIFaddbegin \DIFadd{code injection }\DIFaddend attacks by DMA-capable devices.
...
\begin{enumerate}
    \item The \kva{} of a kernel buffer filled with malicious code (e.g., a ROP attack). Given the device is using \DIFaddbegin \DIFadd{an }\DIFaddend \iova, the attacker needs to obtain the buffer's \kva{}, for example, by observing leaked pointers. 
...
Under the hypothesized circumstance, and without additional information, a malicious device has \DIFdelbegin \DIFdel{practically no valid attack options}\DIFdelend \DIFaddbegin \DIFadd{no viable attack options.
Namely, all three attributes are required for a code injection attack}\DIFaddend .
