...
We \DIFdelbegin \DIFdel{use a 28 core }\DIFdelend \DIFaddbegin \DIFadd{used a 28-core }\DIFaddend Dell PowerEdge R730 server, with Ubuntu 18.04 (kernel version 5.0), as our victim machine. This server is equipped with an Intel VT-d IOMMU, a Broadcom NetXtreme BCM5720 Gigabit Ethernet NIC, a Mellanox Technologies ConnectX-4 Ethernet NIC\DIFaddbegin \DIFadd{, }\DIFaddend and VIA Technologies, Inc. VT6315 Series Firewire Controller. \DIFdelbegin \DIFdel{An identical machine connected }\DIFdelend \DIFaddbegin \DIFadd{We connected an identical machine }\DIFaddend to the victim via a FireWire cable\DIFdelbegin \DIFdel{acts }\DIFdelend \DIFaddbegin \DIFadd{, to act }\DIFaddend as the attacker. 

\smallskip
\noindent\textbf{Executed \DIFdelbegin \DIFdel{attacks}\DIFdelend \DIFaddbegin \DIFadd{Attacks}\DIFaddend .}
We executed the \textit{\DIFdelbegin \DIFdel{Ring Flood}\DIFdelend \DIFaddbegin \DIFadd{RingFlood}\DIFaddend } attack on the \texttt{skb\_shared\_info} structure to inject and run malicious code in the kernel.
...
To complete the attack, we \DIFdelbegin \DIFdel{need to place }\DIFdelend \DIFaddbegin \DIFadd{needed }\DIFaddend a JOP~\cite{BJFL11} gadget that performs \texttt{\%rsp = \%rdi + const}. We \DIFdelbegin \DIFdel{locate }\DIFdelend \DIFaddbegin \DIFadd{located }\DIFaddend such a gadget using the ROPgadget tool~\cite{ROPgadget}.
