...
We claim that the way all state-of-the-art OSs treat I/O devices leads to wrong utilization of the IOMMU, and as a result makes them vulnerable. The reason for the numerous security vulnerabilities is the DMA API it self; writing a driver, that wont be a security burden is a challenge.\newline 
...
\newline
Previous works have looked at sub-page vulnerability and shown that most popular kernels like MacOS, Windows and FreeBSD have a poor IOMMU hygiene; namely sharing address space between devices, mapping to both read and write indiscriminately or eschewing IOMMU altogether\cite{thunder}. The sole bastion of security seemed to be the Linux Kernel, with careful mappings, per device address spaces and IOMMU initialized correctly and by default. And additional software/HW mitigations like KASLR and NX-Bit. To date and to the best of our knowledge, no complete DMA attacks\footnote{A previous NDSS paper\cite{thunder}, has attempted at an attack on a Linux network devices, but the description was incomplete as the attack, as described was infeasible. We actually use it as an example in section \ref{sec:mmo}} were demonstrated against IOMMU enabled Linux Kernel. In this paper we:
\begin{enumerate}
