...
We ignore the NDSS paper, the Linux attacks it mentions are at best half backed, which makes our contribution valid. We will still need to address it because the whole \subpage{} thing; but really its the Copy paper that introduced that notion and this is how we should frame it. The steps are \#1 Write paper as if NDSS doesn't exist. \#2 Add some defensive text when \#1 is complete}

\begin{enumerate}
    \item Introduce Compound DMA attacks vs. straightforward -- add in intro and contribution list.
    \item Connect type (c) opportunity gaining vulnerability is not connected to paper...
    add examples.
    ...
    \item change bnx2 to tg3 (NetXtreme BCM5720)- (napi\_alloc\_frag - type (c))
\end{enumerate}
