...
\textcolor{red}{Unify 5 \& 6 under one section}
...
Currently, human expert input is needed to determine which functions are truly \emph{hazardous}. As a result \tool determines type (a) vulnerabilities automatically and types (b) and (c) only with sufficient human expert input. \textcolor{olive}{As we show in Sec.~\ref{sec:shinfo}, where additional actions are needed to achieve actionable \oportunity{}.}
