...
The output starts from the impact evaluation \textcolor{red}{(e.g., are there any exposed callback pointers)} and continues with pertinent code lines. %\sout{Starred lines contain script analysis}.
%\adam{what is script analysis? Who said anything about a script?}. 
In Fig.~\ref{fig:tool_example}, line 7 tells us that a single callback pointer is mapped in the mapped \texttt{nvme\_fc\_fcp\_op} data structure (i.e., \texttt{fcp\_req.done}) and line 8 tells us that it is possible to spoof another 931 callback pointers\textcolor{red}{, by rewriting pointers to data structures that contain callback pointers}.
...
\textcolor{red}{This example demonstrates the recursive nature of the script. Line 1 identifies the suspect function call \texttt{dma\_map\_single}, Line 2 shows the declaration of the mapped pointer (i.e, \texttt{ptr}) and line 3 shows the pointer type. Lines 4-6 repeat the same pattern now with \texttt{fc\_dma\_map\_single} and \texttt{op}. \tool identifies that a data structure is exposed and searches for exposed callback pointers. The findings are then shown; line 7 counts the number of directly exposed callback pointers, and line 8 shows how many callback may be potentially spoofed.}
...
We find that 13\%~(line 1 in Fig.~\ref{fig:tool_example}) of the drivers expose data structures via type (a) vulnerabilities whereas 60\%~(lines 2,7 in Fig.~\ref{fig:tool_example}) expose data structures via type (b) vulnerabilities. Namely, 13\% are vulnerable due to driver bugs and 60\%~ of drivers are vulnerable due to OS design choices. \textcolor{red}{In the case of the Linux kernel the most common OS design vulnerability is } \sout{We dedicate special attention to} struct \shinfo, which is used ubiquitously in Linux networking. This data structure is \textit{always} located on the same page as the \texttt{skb->data}, and it also contains a callback pointer. We discuss the vulnerabilities related to \shinfo in Sec.~\ref{sec:linux_net}. We find that more than 50\% of the dma-map calls either directly map the \texttt{skb->data} or use the \texttt{build\_skb} API (lines 2 \& 7 in Table ~\ref{tab:static_analysis}), which exposes \shinfo. The OS provides this data structure layout and API rather than it being an isolated driver bug. Additionally, we find 19 data structures that are exposed via APIs that store \texttt{private} data structures on the same page as vulnerable meta-data, e.g., netdev\_priv, aead\_request\_ctx and scsi\_cmd\_priv.

\textcolor{blue}{In addition to type (a) and (b) vulnerabilities, \tool has flagged 344 cases where a type (c) vulnerability is present. Our analysis also finds three instances where the stack pointer is mapped, potentially simplifying the execution of a ROP attack.}

\textcolor{red}{In total we find 742 dma-map call (i.e., 72.8\% of all dma-map calls) have a potential vulnerability of which 344 have a Type C vulnerability.}
