...
We build a static code analysis tool inspired by MMO to flag high-risk device drivers. We use our tool on Linux Kernels 4.15 and 5.0 and identify numerous device drivers at risk for high impact DMA attacks such as privilege escalation by code injection and a full memory dump. Specifically, using the tool, we find that 11\% of the device drivers expose sensitive callback pointers, where as 60\% of device drivers are vulnerable to DMA attacks due to OS design choices. 
