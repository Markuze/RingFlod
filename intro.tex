...
Accordingly, in this paper, we first categorize the four types\adam{who says there are four? first time this comes up} of the \subpage{} vulnerability, providing insight into the structure of DMA vulnerabilities.
Then, to exploit these vulnerabilities, we present a schema for identifying viable attack vectors by malicious DMA capable devices. Specifically, we introduce a trifecta of vulnerability attributes, which is sufficient to layout code injection attacks\adam{should say the scope is code execution earlier and explicitly} (Sec. \ref{sec:mmo}):

\begin{itemize}
    \item A kernel virtual address of a kernel buffer filled with malicious executable code, a \mabaf.
    \item Write access to a function callback pointer.\adam{say that it's inside a struct on the page?}
...
\end{itemize} 
\adam{it looks strange to have these bullets, but no listing of the 4 vuln types. The characterizations should be treated consistently}
...
\begin{itemize}
    \item We build a static code analysis tool (\tool) \adam{can you give it a better name? not only is this REALLY generic, but also: \url{https://www.merriam-webster.com/dictionary/scat}} that flags high-risk device drivers. That is, \tool identifies drivers that expose callback pointers.
