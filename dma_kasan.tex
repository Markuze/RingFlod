\subsection{Run-time Analysis Tool}\label{sec:dma-kasan} 
...
\tool is static analysis tool that may fail to follow a mapped variable due to potential code obfuscation like function pointers, macros and others, potentially resulting in a false negative result. False positive may happen in a rare case\footnote{Obviously excluding coding bugs.}\adam{the footnote and its purposes are't clear. Bugs by who? Tool writers (you) or kernel devs? And what exactly is the point?} where the mapped  data  structure  crosses  a  page  boundary,  \tool may flag a callback function which may not be exposed since it resides on a different page.\adam{explain why this can happen}