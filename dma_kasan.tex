...
\tool is static analysis tool that may fail to follow a mapped variable due to potential code obfuscation like function pointers, macros and others, potentially resulting in a false negative result. False positive may happen in a rare case\adam{the footnote and its purposes are't clear. Bugs by who\textcolor{red}{\textbf{m}}? Tool writers (you) or kernel devs? And what exactly is the point?}\SV{Removed} where the mapped  data  structure  crosses  a  page  boundary. In this case, \tool may flag a callback function, which may not be exposed, since it resides on a different page. \textcolor{violet}{Namely, only part of a data structure is accessible to the device due to the \subpage{} vulnerability at the mapped page whereas the callback pointer resides on a different page which is not accessible to the device.}\adam{explain why this can happen}\SV{Is this what you were looking for?}