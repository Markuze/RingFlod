...
% DMA attacks are real - now deal with it...
...
The blame is not with the device drivers alone. With the existing API used for I/O operations, it is difficult not to create a sub-page vulnerability. And even, well written drivers can be subverted by the OS (e.g, bnx2 by deferred protection). 
...
%we know this to be untrue. The  To a lesser degree, the  This is dangerous, because this seems to encourage building an \skb around a dma mapped buffer; exposing \shinfo. 
%In fact, we were looking for this function when working on a list of vulnerable device drivers.  Using the staunch, \textit{dma\_\{un\}map\_page} or using tools like \textit{dma\_page\_pool}.
