...
In this section we organize and categorize the risks associated with DMA operations, providing the building blocks for reasoning about code-injection attacks.
...
Finaly, we identify a set of three vulnerability attributes that make it possible for a malicious device to exploit a \subpage{} vulnerability and execute a viable code injection attack.
...
For example, a full memory dump is possible, when an attacker is able to modify data pointers before they are mapped, causing the driver to map arbitrary kernel addresses.
...
Nevertheless, the most significant potential consequence of our attacks is privilege escalation via code injection, which allows attackers to execute arbitrary code with kernel privileges. Indeed, this is the focus of our paper. 
