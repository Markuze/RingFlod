...
\begin{enumerate}
    ...
    The device will retain access to the physical page as long as a single valid \iova{} exists. This means that the device can temper with memory regions that should no longer be accessible to it. We discuss the practical implications of multiple \iova indicating the same page in Sec.~\ref{sec:timely}.
